...
TITLE         VARCHAR2(50) NOT NULL,                  -- 리뷰 제목
WRITER        VARCHAR2(30) NOT NULL,                  -- 작성자 닉네임
CONTENT       VARCHAR2(500) NOT NULL,                 -- 리뷰 내용
CREATED_AT    TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,-- 작성일
UPDATED_AT    TIMESTAMP,                              -- 수정일 (선택)
VIEW_C        NUMBER(10),                           -- 조회수 (선택)

CONSTRAINT CK_CO_MEMBER_ID_FORMAT CHECK (REGEXP_LIKE(MEMBER_ID, '^[A-Za-z0-9]+$'))
);
...
ALTER TABLE COMMUNITY MODIFY VIEW_C NUMBER DEFAULT 0;
----------------------------------------------------------
